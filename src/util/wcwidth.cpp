...
/// Binary search @p table (sorted by .first) for a range containing @p cp.
bool bisearch(char32_t cp, const interval table[], size_t n) {
  if (cp < table[0].first || cp > table[n - 1].last)
    return false;
  size_t lo = 0, hi = n - 1;
  while (lo <= hi) {
    size_t mid = (lo + hi) / 2;
...
// Combining characters (Unicode General Category Mn | Mc | Me) and
// other zero-width codepoints: ZWJ, ZWNJ, ZWSP, soft hyphen,
// Hangul fillers, and variation selectors.  Adapted from the
// public-domain mk_wcwidth() by Markus Kuhn.
const interval zero_width[] = {
    ...
    {0x0CD5, 0x0CD6},   {0x0CE2, 0x0CE3},   {0x0CF3, 0x0CF3},
    ...
    {0x1CF7, 0x1CF9},   {0x1DC0, 0x1DFF},   {0x200B, 0x200F},
    {0x2028, 0x202E},   {0x2060, 0x2064},   {0x2066, 0x206F},
    {0x20D0, 0x20F0},   {0x2CEF, 0x2CF1},   {0x2D7F, 0x2D7F},
    {0x2DE0, 0x2DFF},   {0xA66F, 0xA672},   {0xA674, 0xA67D},
    ...
    {0x110B0, 0x110BA}, {0x110C2, 0x110C2}, {0x110F0, 0x110F2},
    ...
    {0x112F0, 0x112F0}, {0x11300, 0x11303}, {0x1133B, 0x1133C},
    ...
    {0x11A59, 0x11A5B}, {0x11A8A, 0x11A99}, {0x11C2F, 0x11C36},
    ...
    {0x11F3E, 0x11F42}, {0x11F45, 0x11F45}, {0x11F4A, 0x11F4A},
    {0x13430, 0x1345F}, {0x13440, 0x13440}, {0x13447, 0x13455},
    ...
    {0x1E5EE, 0x1E5EF}, {0x1E66F, 0x1E6E0}, {0x1E8D0, 0x1E8D6},
    {0x1E944, 0x1E94A}, {0xE0100, 0xE01EF},
};

// East Asian Wide / Fullwidth codepoints (East Asian Width property
// values W or F).  These occupy two terminal columns instead of one.
const interval wide[] = {
    {0x1100, 0x115F},   {0x2329, 0x232A},   {0x2E80, 0x303E},
    {0x3040, 0x33BF},   {0x3400, 0x4DBF},   {0x4E00, 0xA4CF},
    {0xA960, 0xA97C},   {0xAC00, 0xD7A3},   {0xF900, 0xFAFF},
    {0xFE10, 0xFE19},   {0xFE30, 0xFE6F},   {0xFF01, 0xFF60},
    {0xFFE0, 0xFFE6},   {0x1B000, 0x1B0FF}, {0x1B100, 0x1B12F},
    {0x1F200, 0x1F2FF}, {0x20000, 0x2FFFF}, {0x30000, 0x3FFFF},
};

}  // anonymous namespace

/** @copydoc vtui::utf::cell_width */
int cell_width(char32_t cp) {
  // C0 and C1 controls (excluding NUL which we allow).
  if (cp == 0)
    return 0;
  if (cp < 0x20 || (cp >= 0x7F && cp < 0xA0))
    return -1;

  if (bisearch(cp, zero_width, std::size(zero_width)))
    return 0;

  if (bisearch(cp, wide, std::size(wide)))
    return 2;
...
void Cell::compute_width() {
  int w = utf::cell_width(base);
  width = w > 1 ? static_cast<uint8_t>(w) : 1;
}
